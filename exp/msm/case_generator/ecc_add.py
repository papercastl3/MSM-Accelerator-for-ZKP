...
def point_double_hw(X1, Y1, Z1):
    ...
    two_Y1 = lazy_add(Y1, Y1)  # 로직 변경
    Z3     = mont_mul_lazy_hw(two_Y1, Z1)
    
    YYYY   = mont_mul_lazy_hw(YY, YY)
    # 추가  : YYYY범위 축소
    YYYY   = lazy_subtraction_N_hw(YYYY)
    YYYY   = lazy_subtraction_N_hw(YYYY)
    ...
    with open(outfile, "w") as f:
        ...
        for test_idx in range(NUM_TESTS):
            ...
            if rand_type < 0.01:    # Case 1: P1이 영점 (1%)
                ...
                case_name = "Case 5: 일반 점 덧셈 (Mixed Add)"
                x1 = random.randint(1, N - 1)
                y1 = random.randint(1, N - 1)
                x2 = random.randint(1, N - 1)
                y2 = random.randint(1, N - 1)
            ...
            if true_x3 != our_x3 or true_y3 != our_y3:
                # [수정] 에러 발생 시 상세 정보 출력 (로직 변경 없음)
                print(f"\n[❌ 맵핑 에러 발생!] Test Index: {test_idx}")
                print(f"-> 에러 발생 분기 : {case_name}")
                print("-> 입력된 Affine 좌표:")
                print(f"   P1(x1, y1) = ({x1:x}, {y1:x})")
                print(f"   P2(x2, y2) = ({x2:x}, {y2:x})")
                print("-> 예상 결과 (Python Affine):")
                print(f"   ({true_x3:x}, {true_y3:x})")
                print("-> 하드웨어 출력 변환 결과 (Our Affine):")
                print(f"   ({our_x3:x}, {our_y3:x})")
                print("-> 하드웨어 원본 출력 (Our Jacobian):")
                print(f"   X3 = {X3:x}\n   Y3 = {Y3:x}\n   Z3 = {Z3:x}")
                print("-" * 60)
