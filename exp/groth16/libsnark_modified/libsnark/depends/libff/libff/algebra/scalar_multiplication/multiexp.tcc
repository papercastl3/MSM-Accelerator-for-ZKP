...
static void fpga_dump_bigint_be(std::ostream &os, const BigIntT &v)
{
    os << std::hex << std::setfill('0');

    for (int limb = BigIntT::N - 1; limb >= 0; --limb) {
        os << std::setw(16) << v.data[limb];
    }
}
...
#pragma omp parallel for
    ...
    /*
     * FPGA RX window order:
     *   window_order[0]  = scalar MSB side 11-bit window
     *   ...
     *   window_order[13] = last 11-bit window
     *   window_order[14] = first 10-bit window
     *   ...
     *   window_order[23] = scalar LSB side 10-bit window
     *
     * 14 * 11 + 10 * 10 = 254 bits.
     *
     * DMA/RX layout still reserves 2048 bucket slots per window because the
     * hardware stream emits one 2048-row bucket zone per window. 10-bit windows
     * use only bucket indices 1..1023; indices 1024..2047 are ignored.
     */
    const size_t WINDOW_BITS[WINDOW_COUNT] = {
    ...
    const size_t MAX_BITS_PER_WINDOW = 11;
    const size_t BUCKETS_PER_WINDOW = 1 << MAX_BITS_PER_WINDOW; // 2048
    ...
    for (size_t i = 0; i < in_count; ++i) {
        ...
        /*
         * FPGA point input slicing currently interprets:
         *   tdata[511:256] as X
         *   tdata[255:0]   as Y
         *
         * AXI DMA maps the lower DDR address to the lower tdata bits.
         * Therefore store Y first and X second in memory.
         */
        memcpy(pt,
               &dma_points[i].Y,
    ...
    /*
     * Debug dump: logical FPGA inputs immediately before starting DMA.
     *
     * These files are written in normal big-endian hex text.
     *
     *   fpga_input_points_mont.hex:
     *     X_mont_Y_mont
     *
     *   fpga_input_scalars.hex:
     *     scalar
     *
     * Note:
     *   The actual PT DMA memory is stored as Y then X to compensate for the
     *   current FPGA point input slicing.  This dump is intentionally written
     *   in logical X_Y order so it can be compared with the expected generator.
     */
    // {
    //     std::ofstream point_dump("fpga_input_points_mont.hex");
    //     std::ofstream scalar_dump("fpga_input_scalars.hex");

    //     if (!point_dump) {
    //         std::cerr << "[FPGA DMA] failed to open fpga_input_points_mont.hex"
    //                   << std::endl;
    //     }

    //     if (!scalar_dump) {
    //         std::cerr << "[FPGA DMA] failed to open fpga_input_scalars.hex"
    //                   << std::endl;
    //     }

    //     if (point_dump) {
    //         for (size_t i = 0; i < in_count; ++i) {
    //             fpga_dump_bigint_be(point_dump, dma_points[i].X);
    //             point_dump << '_';
    //             fpga_dump_bigint_be(point_dump, dma_points[i].Y);
    //             point_dump << '\n';
    //         }

    //         std::cerr << "[FPGA DMA] input point dump written: "
    //                   << "fpga_input_points_mont.hex, lines="
    //                   << in_count << std::endl;
    //     }

    //     if (scalar_dump) {
    //         for (size_t i = 0; i < in_count; ++i) {
    //             fpga_dump_bigint_be(scalar_dump, dma_scalars[i]);
    //             scalar_dump << '\n';
    //         }

    //         std::cerr << "[FPGA DMA] input scalar dump written: "
    //                   << "fpga_input_scalars.hex, lines="
    //                   << in_count << std::endl;
    //     }
    // }
    ...
    for (size_t iter = 0; iter < DMA_ITERATIONS; ++iter) {
        ...
        for (size_t i = 0; i < in_count; ++i) {
            ...
            mm2s0[i].control =
                (is_first ? BD_CTRL_TXSOF : 0) |
                (is_last  ? BD_CTRL_TXEOF : 0) |
                PT_BYTES;
        ...
        /*
        * S2MM:
        * Receive one whole iteration as one SG BD.
        *
        * PL sends 8 windows * 2048 buckets * 96B.
        * TLAST is expected once at the end of this iteration.
        */
        s2mm0[0].next_desc =
     ...
     *
     * 주의:
     *   fpga_buckets[0]  = MSB 쪽 첫 번째 11-bit window
     *   fpga_buckets[13] = 11-bit 마지막 window
     *   fpga_buckets[14] = 10-bit 첫 번째 window
     *   fpga_buckets[23] = LSB 쪽 마지막 10-bit window
     *
     * iter 0 RX + 0x000000 -> MSB-order window 0~7
     * iter 1 RX + 0x180000 -> MSB-order window 8~15
     * iter 2 RX + 0x300000 -> MSB-order window 16~23
     */
    for (size_t iter = 0; iter < DMA_ITERATIONS; ++iter) {
        const size_t rx_iter_base = iter * RX_BYTES_PER_ITER;
        ...
        for (size_t local_w = 0; local_w < OUT_ZONES_PER_ITER; ++local_w) {
            ...
            for (size_t b = 0; b < BUCKETS_PER_WINDOW; ++b) {
                ...
                /*
                 * FPGA result memory layout observed through S2MM:
                 *   rx + 0  : Z
                 *   rx + 32 : Y
                 *   rx + 64 : X
                 *
                 * Normalize to fpga_buckets[w][b] = X/Y/Z.
                 */
    ...
    /*
     * Debug dump: normalized FPGA output buckets.
     *
     * The S2MM memory is observed as:
     *   rx + 0  : Z
     *   rx + 32 : Y
     *   rx + 64 : X
     *
     * fpga_buckets[][] has already been normalized to X/Y/Z above.
     * This dump is therefore written as:
     *   X_mont_Y_mont_Z_mont
     */
    // {
    //     std::ofstream out_dump("fpga_output_buckets_mont.hex");

    //     if (!out_dump) {
    //         std::cerr << "[FPGA DMA] failed to open fpga_output_buckets_mont.hex"
    //                   << std::endl;
    //     } else {
    //         for (size_t w = 0; w < WINDOW_COUNT; ++w) {
    //             for (size_t b = 0; b < BUCKETS_PER_WINDOW; ++b) {
    //                 fpga_dump_bigint_be(out_dump, fpga_buckets[w][b].X);
    //                 out_dump << '_';
    //                 fpga_dump_bigint_be(out_dump, fpga_buckets[w][b].Y);
    //                 out_dump << '_';
    //                 fpga_dump_bigint_be(out_dump, fpga_buckets[w][b].Z);
    //                 out_dump << '\n';
    //             }
    //         }

    //         std::cerr << "[FPGA DMA] output bucket dump written: "
    //                   << "fpga_output_buckets_mont.hex, lines="
    //                   << (WINDOW_COUNT * BUCKETS_PER_WINDOW)
    //                   << std::endl;
    //     }
    // }
    ...
    for (size_t w = 0; w < WINDOW_COUNT; ++w) {
        const size_t buckets_this_window = size_t(1) << WINDOW_BITS[w];

        for (size_t b = 1; b < buckets_this_window; ++b) {
...
#pragma omp parallel for
    ...
    for (size_t w = 0; w < WINDOW_COUNT; ++w) {
        ...
        for (size_t b = buckets_this_window - 1; b > 0; --b) {
    ...
    // =====================================================================
    // 6. 최종 aggregation
    //    window_results[]는 이미 MSB -> LSB 순서이다.
    //    다음 window를 더하기 전에, 그 다음 window의 bit-width만큼 shift한다.
    // =====================================================================
    ...
    for (size_t w = 0; w < WINDOW_COUNT; ++w) {
        if (final_nonzero) {
            for (size_t shift = 0; shift < WINDOW_BITS[w]; ++shift) {
